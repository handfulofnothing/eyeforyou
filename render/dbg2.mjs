...
const rows = await p.evaluate(async ([a, z, sel]) => { const out = []; const e = document.querySelector(sel);
  for (let t = a; t <= z + 1e-9; t += 1 / 60) { const s = await window.seek(t); out.push([t.toFixed(3), s.toFixed(3), e.getAttribute('transform')]); } return out; }, [a, z, sel]);
